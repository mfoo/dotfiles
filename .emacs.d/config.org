...
org mode's functionality available within the configuration file itself. More information can be found in the [[Installation
 instructions][Installation instructions]] section.
...
Note: Enabling line numbers globally is not perfect as it includes things like lists of files when using =C-x f=. This
doesn't really make sense. It would be nice to only do this for modes that are used when editing text. Is there
something I can hook into to improve this?
...
Don't make temporary backup files. These are more annoying than they are useful. I tend to save a lot because I use
...
remember the shortcut for something.
