...
#+END_SRC
* Blog
Now that we've configured =org-mode=, let's configure my blog. I always have this in the same location on machines:
=~/repositories/mfoot.com=. I can therefore add a custom publishing section to =org= (taken from the [[http://orgmode.org/worg/org-tutorials/org-jekyll.html][org-jekyll
documentation]]):

TODO: See if the following have any effect:
#+BEGIN_SRC emacs-lisp
...
This essentially defines an export with the name =mfoot.com= that exports both the posts and the static content.

TODO: update this when I understand it better. Currently it's just copy pasted and modified from the docs.
