...
Configure =org-mode= to use a variable pitch font for prose but a fixed pitch font for various blocks.
...
Since we're using a variable pitch font we enable =visual-line-mode= so we don't hard wrap all lines. This means when
making the screen wider the text will reflow.
