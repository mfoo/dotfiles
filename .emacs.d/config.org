...
(autoload 'git-commit-jira-prefix-init
          (concat user-emacs-directory "git-commit-jira-prefix.el"))
...
I never use Linux's mail system so I don't want emacs to tell me if I have new emails. This removes the "Mail" link from
the mode line indirectly:
